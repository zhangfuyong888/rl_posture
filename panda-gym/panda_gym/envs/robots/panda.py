...
def get_Jacobian(self) -> tuple[np.ndarray, np.ndarray]:
    """计算末端执行器在当前位置下的雅可比矩阵（线速度部分和角速度部分）

    Returns:
        J_linear (np.ndarray): 线速度雅可比 (3 x 7)
        J_angular (np.ndarray): 角速度雅可比 (3 x 7)
    """
    robot_id = self.sim._bodies_idx[self.body_name]
    link_index = self.ee_link
    local_position = [0.0, 0.0, 0.0]  # 在末端 link 局部坐标系中点

    joint_angles = [float(self.get_joint_angle(i)) for i in range(7)]
    joint_velocities = [0.0] * 7
    joint_accelerations = [0.0] * 7

    J_lin, J_ang, _ = self.sim.physics_client.calculateJacobian(
